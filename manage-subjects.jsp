...
<html lang="en">
    <head>
        ...
        <title>TMSL | Manage Subjects</title>
...
<%try 
		{
			...
			int count=0;
			rs = stmt.executeQuery("SELECT * from tblsubjects");
			while(rs.next())
			{
				count++;
%>	
                                                    ...
                                                    <td><%if(rs.getString("UpdationDate")==null)
                                                    {%> <%="-"%><%}
                                                    else
                                                    {
                                                    %><%=rs.getString("UpdationDate") %><% } %></td>
													<td>
	<a href="edit-subject.jsp?id=<%=rs.getString("id")%>"><i class="fa fa-edit" title="Edit Record"></i></a>
	<a href="delete-subject.jsp?id=<%=rs.getString("id") %>" onclick="return confirm('Are you sure you want to permanently delete this record?.');"><i class="fa fa-trash color-danger" title="Delete Record"></i></a>
</td>
</tr>
<% } %>
