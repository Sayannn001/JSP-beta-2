...
<%try 
		{
			...
			int count=0;
			rs = stmt.executeQuery("(SELECT tblstudents.StudentName, tblstudents.RollId, tblstudents.RegDate, tblstudents.StudentId, tblstudents.Status, tbldept.DeptName, tbldept.Short FROM tblstudents JOIN tbldept ON tbldept.id = tblstudents.ClassId);");
			while(rs.next())
			{
				count++;
%>	
...
<td>
	<a href="edit-student.jsp?StudentId=<%=rs.getString("StudentId")%>"><i class="fa fa-edit" title="Edit Record"></i></a>

<a href="#" onclick="return alert('Student records are permanent. They can only be edited or added. Removal is blocked.');"><i class="fa fa-trash color-danger" title="Delete Record"></i></a>
</td>
</tr>
<% } %>
