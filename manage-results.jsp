...
<html lang="en">
    <head>
        ...
        <title>TMSL | Manage Students</title>
...
<%try 
		{
			...
			int count=0;
			rs = stmt.executeQuery("SELECT  distinct tblstudents.StudentName,tblstudents.RollId,tblstudents.RegDate,tblstudents.StudentId,tblstudents.Status,tbldept.DeptName,tbldept.Short from tblresult join tblstudents on tblstudents.StudentId=tblresult.StudentId  join tbldept on tbldept.id=tblresult.ClassId;");
			while(rs.next())
			{
				count++;
%>	
                                                    ...
                                                    <td><%=rs.getString("RollId")%></td>
                                                    <td><%=rs.getString("DeptName")%>(<%=rs.getString("Short")%>)</td>
...
<td>
	<a href="edit-result.jsp?StudentId=<%=rs.getString("StudentId")%>"><i class="fa fa-edit" title="Edit Record"></i></a>
	<a href="delete-result.jsp?StudentId=<%=rs.getString("StudentId") %>" onclick="return confirm('Are you sure you want to permanently delete this record?.');"><i class="fa fa-trash color-danger" title="Delete Record"></i></a>
</td>
</tr>
<% } %>
